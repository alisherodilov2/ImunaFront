...
import RepotExpenseShow from './RepotExpenseShow'
...
const RepotExpense = () => {
    ...
    const getData = async (data: any) => {
        try {
            setLoad(() => true)
            let res = await axios.get(`/expense/repot?start_date=${data?.start_date}&end_date=${data?.end_date}&branch_id=${data?.branch?.value ?? ''}`)
    ...
    const getDataShow = async (date: any, branchId: any) => {
        try {
            setLoading(() => true)
            let res = await axios.get(`/expense/repot/show?date=${date}&branch_id=${branchId}`)
            const { result } = res.data
            // setData(() => result)
            // console.log(result);
            setItem(() => result)
                        ...
                        columns={[
                            {
                                ...
                                renderItem: (value: any, data: any) => {
                                    return <td
                                        onClick={() => {
                                            getDataShow(data?.date, data?.owner_id)
                                        }}
                                    >
                                        {value}
                                    </td>
                            ...
                            },
                            {
                                ...
                                renderItem: (value: any, data: any) => {
                                    return <td
                                        onClick={() => {
                                            getDataShow(value, data?.owner_id)
                                        }}
                                    >
                                        {value}
                                    </td>
