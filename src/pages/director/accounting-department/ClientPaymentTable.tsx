...
const ClientPaymentTable = () => {
    ...
    const allShow = async (data?: any) => {
        try {
            setLoad(() => true)
            let res = await axios.get(`/repot?status=pay_all_client&start_date=${data?.start_date ?? ''}&end_date=${data?.end_date ?? ''}&branch_id=${data?.branch?.value ?? ''}&person_id=${data?.person?.value ?? ''}&full_name=${data?.full_name ?? ''}&phone=${data?.phone ?? ''}&data_birth=${data?.data_birth ?? ''}`)
            const { result } = res.data
            setData(() => result)
            // setItem({})
            // setModaledit(true)
