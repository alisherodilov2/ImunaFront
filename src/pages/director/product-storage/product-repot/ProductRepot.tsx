...
import ProductRepotShow from './ProductRepotShow'
...
const ProductRepot = () => {
    ...
    const getData = async (data: any) => {
        try {
            setLoad(() => true)
            let res = await axios.get(`/product/repot?start_date=${data?.start_date}&end_date=${data?.end_date}`)
    ...
    const getDataShow = async (date: any) => {
        try {
            setLoading(() => true)
            let res = await axios.get(`/product/repot/show?date=${date}`)
            const { result } = res.data
            // setData(() => result)
            // console.log(result);
            setItem(() => result)
