...
const MuolajaChart = () => {
    ...
    const value = (statisticaData?.mulojagayozilgnlar ?? 0)?.toFixed(2); // Fallback to 0 if undefined
    ...
    const series = [(value / maxValue) * 100]; // Convert the value to percentage
    return (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center',  }}>
            <Chart options={options} series={series} type="radialBar"  width={750}/>
        </div>
    );
};
