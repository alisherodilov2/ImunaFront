...
const RepotDesktop = () => {
    ...
    const show = async (id: any) => {
        try {
            setLoad(() => true)
            let res = await axios.get(`/referring-doctor/show/${id}?start_date=${referringDoctorData?.start_date}&end_date=${referringDoctorData?.end_date}&is_repot=1`)
            const { result } = res.data
                ...
                <div className="d-flex my-1 gap-3">
                    <form className='row w-100'>
                        ...
                        <div className="col-2">
                            ...
                            disabled={isLoading}
                            min={referringDoctorData?.start_date} onChange={(e: any) => {
                                let value = e.target.value
                                if (value && value.length > 0) {
