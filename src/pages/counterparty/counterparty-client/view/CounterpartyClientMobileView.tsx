...
const CounterpartyClientMobileView = () => {
    ...
    const showClient = async (id: any, status?: any, full_name?: any) => {
        try {
            setLoad(() => true)
            let res = await axios.get(`/referring-doctor/treatment?show_id=${id}&status=${status}&full_name=${full_name ?? ''}`)
            const { result } = res.data
            setData(() => result)
            ...
            <div className="container-fluid flex-grow-1 container-p-y size_16 ">
                <div className='my-2  d-flex justify-content-between gap-2 '>
                    <Input placeholder='F.I.O Izlash...'
                        ...
                        onChange={(e: any) => {
                            showClient(id, statusGenerate(tab), e.target.value?.trim().toLowerCase())
                            setSearch((res: any) => {
                                return {
                                    ...res,
                ...
                <div>
                    <Select
                        ...
                        onChange={(e: any) => {
                            ...
                            // })
                            // path('counterparty-client/')
                            showClient(e.value, 'ambulator')
                            setTab(0)
                            setSearch(() => {
                                return {
                                    full_name: ''
                    ...
                    load ? '' :
                        <div className="card my-2">
                            <div className="d-flex justify-content-between align-items-center my-1">
                                <p className='mb-0'>
                                    <strong>{fullName(data?.doctor)} </strong>
                                    <br />
                                    <span>{data?.doctor?.workplace}</span>
                                </p>
                                <p className='mb-0'>
                                    {item?.client?.length}
                                </p>
                            ...
                            {
                                !load && (data?.data?.client?.length == 0 || data?.data == null) ? <div className='bg-white rounded p-1 text-center d-flex  align-items-center gap-3 justify-content-center'>
                                    <FaBoxOpen size={44} />
                                    <h4 className='mb-0'>Malumot topilmadi</h4>
                                </div> : data?.data?.client?.map((clientItem: any) => (
                                    <div className="border border-primary rounded p-2 my-1">
                                        ...
                                        <p >+998 {phoneFormatNumber(data?.data?.phone)}</p>
                                        {
                                        ...
                                        {/* <div className="d-flex my-2 gap-3">
                                        {
                                            [1, 2, 3, 4].map((dayItem: any, index: number) => (
                                                <p className='d-flex gap-1'>
                                                    <span> {index + 1}</span>
                                                    <FaCircle size={24} className='text-warning' />
                                                </p>
                                            ))
                                        }
                                    </div>
                                    <div className="d-flex my-2 gap-3">
                                        {
                                            [1, 2, 3, 4].map((dayItem: any, index: number) => (
                                                <p className='d-flex gap-1'>
                                                    <span> {index + 1}</span>
                                                    <FaCircle size={24} className='text-danger' />
                                                </p>
                                            ))
                                        }
                                    </div> */}
...
export default CounterpartyClientMobileView