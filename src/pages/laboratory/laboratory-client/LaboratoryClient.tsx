...
${content}</div>
    ...
    const smsAccept = async (id: any) => {
        try {
            setSendLoading(() => true)
            let res = await axios.get('/client/laboratory/sms-send/' + id)
            const { result } = res.data
        } catch (error) {
                ...
                <div className="card" style={{
                    ...
                    <Table
                        ...
                        extraButton={function (item: any) {
                            const { id } = item.client_item?.at(-1)
                            return <>
                                ...
                                <Link to={`/client-check/${id}`} className='btn btn-warning btn-sm'
                                    onClick={() => {
                                        bloodtestAccept(id)
                                    }}
                                >
                        ...
                        columns={[
                            {
                                ...
                                renderItem: (value: any, data: any) => {
                                    const { is_result } = data?.client_item?.at(-1)
                                    return <td key={data.index} className={`${is_result ? 'bg-success text-white' : 'bg-warning text-white'}`}>
                                        <span>
                            ...
                            },
                            {
                                ...
                                render: (value: any, data: any) => {
                                    const { client_value, is_result } = value?.client_item?.at(-1)
                                    return < >
                                        {`${client_value.service}/${client_value.result}`}
                                    </>
                                }
                            },
                            {
                                ...
                                key: 'full_name',
                                renderItem: (value: any, data: any) => {
                                    const { client_value } = value?.client_item?.at(-1)
                                    return <td onClick={() => {
...

                            {
                                ...
                                key: 'sms_',
                                render: (value: any, data: any) => {
                                    return <input type="checkbox" checked={value?.client_item?.at(-1)?.is_sms} className=' form-check-input'
                                        onChange={() => {
                                            smsAccept(value?.client_item?.at(-1)?.id)
                                        }}
