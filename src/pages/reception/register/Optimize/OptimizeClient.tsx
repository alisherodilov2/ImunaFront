...
import { isCashRegItem2, isClientAddExcelFile, isClientCurrentPage, isClientDelete, isClientGet, isClientPageLimit, isClientReceptionGet, isTornametClient } from '../../../../service/reducer/ClientReducer'
...
const OptimizeClient = () => {
    ...
    useEffect(() => {
        ...
        //     dispatch(isClientGet(`?department_id=${department_id}&clienttype_id=${clienttype_id}&per_page=500`))
        // } else {
        //     dispatch(isClientGet('?per_page=500'))
        // }
        dispatch(isClientGet('?per_page=500'))

        // dispatch(isClientReceptionGet('?per_page=500'))
    ...
    const servicePrintChek = async (data: any) => {
        try {
            setLoad(() => true)
            let res = await axios.get(`/client/service-print?id=${data?.id}`)
            const { result } = res.data
            generateCheck({
                target: {
                    ...result,
                            ...
                            <div className="d-flex gap-2">
                                ...
                                <Input type='date'
                                    ...
                                    min={clientData?.start_date} onChange={(e: any) => {
                                        let value = e.target.value
                                        if (value && value.length > 0) {
                                            dispatch(isClientGet(`?start_date=${clientData?.start_date}&end_date=${value}&per_page=500&page=1`))
                                        }
                                    }}
                                ...
                                onKeyDown={
                                    (e: any) => {
                                        ...
                                        if (e.key === 'Enter' && e.target.value?.trim().length > 0) {
                                            ...
                                            dispatch(isClientGet(`?start_date=${clientData?.start_date}&end_date=${clientData?.end_date}&full_name=${e.target.value?.trim().toLowerCase()}&per_page=500&page=1`))
                                        }
                                    }
                                }
                                ...
                                onKeyDown={
                                    (e: any) => {
                                        ...
                                        if (e.key === 'Enter' && e.target.value?.trim().length > 0) {
                                            ...
                                            dispatch(isClientGet(`?start_date=${clientData?.start_date}&end_date=${clientData?.end_date}&phone=${e.target.value?.trim().toLowerCase()}&per_page=500&page=1`))
                                        }
                                    }
                                }
                                value={search?.phone}
                                ...
                                onKeyDown={
                                    (e: any) => {
                                        ...
                                        if (e.key === 'Enter' && e.target.value?.trim().length > 0) {
                                            ...
                                            dispatch(isClientGet(`?start_date=${clientData?.start_date}&end_date=${clientData?.end_date}&person_id=${e.target.value?.trim().toLowerCase()}&per_page=500&page=1`))
                                        }
                                    }
                                }
                                ...
                                onKeyDown={
                                    (e: any) => {
                                        ...
                                        if (e.key === 'Enter' && e.target.value?.trim().length > 0) {
                                            ...
                                            dispatch(isClientGet(`?start_date=${clientData?.start_date}&end_date=${clientData?.end_date}&data_birth=${e.target.value?.trim().toLowerCase()}&per_page=500&page=1`))
                                        }
                                    }
                                }
                        ...
                        <div className={`col-2 ${+user?.setting?.is_reg_status ? '' : 'd-none'}`}>
                            <Select
                                ...
                                onChange={(e: any) => {
                                    ...
                                    dispatch(isClientGet(`?start_date=${clientData?.start_date}&end_date=${clientData?.end_date}&status=${e.value}&per_page=500&page=1`))
                                }}
                        ...
                        <div className="col-4">
                            <Select
                                ...
                                onChange={(e: any) => {
                                    ...
                                    dispatch(isClientGet(`?start_date=${clientData?.start_date}&end_date=${clientData?.end_date}&department_id=${e.value}&per_page=500&page=1`))
                                    // setSearch((res: any) => {
                                    //     return {
                                    //         ...res,
                                    //         department: e
                    ...
                    <Table
                        ...
                        reloadDataFunction={() => {
                            if ((department_id && +department_id > 0) && (clienttype_id && +clienttype_id > 0)) {
                                ...
                                dispatch(isClientGet('?per_page=500&page=1'))
                            }
                        }}
                        top={100}
                        scrollRole={true}
                            ...
                            // },
                            {
                                ...
                                render: (value: any, data: any) => {
                                    ...
                                    return <>
                                        ...
                                        <span className={`text-danger ${total_price - discount - pay_total_price > 0 ? '' : 'd-none'}`}>
                                            /
                                            <NumericFormat displayType="text"
                                                thousandSeparator
                                                decimalScale={2}
...

                            {
                                ...
                                render: (value: any) => {
                                    ...
                                    return <div className="d-flex align-items-center gap-1 flex-wrap ">
                                        {department?.map((item: any) => <p className={`mb-1 text-white badge bg-${item?.is_check_doctor == 'finish' ? 'success' : 'secondary'}`} key={item?.id}>{item?.name}</p>)}
                                    </div>
                                }
                            },
                            {
                                ...
                                render: (value: any) => {
                                    ...
                                    return <div className="d-flex align-items-center gap-1 flex-wrap ">
                                        {service?.map((item: any) => <p className={`mb-1 text-white badge bg-${item?.is_check_doctor == 'finish' ? 'success' : 'secondary'}`} key={item?.id}>{item?.name}</p>)}
                                    </div>
                                }
                            },
                            {
                                ...
                                render: (value: any, data: any) => {
                                    ...
                                    return <div className='my-1'>
                                        {
                                            queue_number?.map((item: any) => <p className={`mb-1 text-white badge bg-${item?.is_check_doctor == 'finish' ? 'success' : 'secondary'}`} key={item?.id}>{item?.queue_number}</p>)
                                        }
                                    </div>
                                }
                            },
