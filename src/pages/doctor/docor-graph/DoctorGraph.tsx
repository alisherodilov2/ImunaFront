...
import DoctorGraphAdd from './DoctorGraphAdd'
                ...
                <div className="d-flex my-2 gap-3">
                        ...
                        <div className="col-2">
                            <Select
                                ...
                                onChange={(e: any) => {
                                    ...
                                    //     }
                                    // })
                                    dispatch(isGraphGet(`?year=${e?.value ?? ''}&department_id=${user?.department?.value ?? ''}&month=${graphData?.month?.value ?? ''}`))
                                }}
                                className="basic-multi-select"
                                classNamePrefix="select"
                                options={
                    ...
                    <div className='d-flex justify-content-center align-items-center gap-1'>
                        ...
                        <button className="btn btn-primary " type="button" onClick={() => {
                            dispatch(isGraphGet(`?year=${graphData?.current_year?.value ?? ''}&week=${graphData?.weekcount - 1}&department_id=${user?.department?.value}`))
                        }}>oldingi</button>
                        <button className="btn btn-primary " type="button" onClick={() => {
                            dispatch(isGraphGet(`?year=${graphData?.current_year?.value ?? ''}&week=${graphData?.weekcount + 1}&department_id=${user?.department?.value}`))
                        }}>Keyingi</button>
                        ...
                        > */}
                        {
                            isSuccess && graphData?.week_data?.length > 0 ?
                                graphData?.week_data?.map((item: any, index: number) => {
                                    ...
                                    const workingday = JSON.parse(user?.department?.working_days ?? "[]")
