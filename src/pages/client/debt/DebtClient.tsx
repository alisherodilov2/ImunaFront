...
const DebtClient = () => {
    ...
    const clientAllData = async (data?: any) => {
        try {
            ...
            setLoad(() => true)
            let res = await axios.get(`/client/all?status=debt&${data ?? ''}`)
                            ...
                            <Input type='date'
                                ...
                                onChange={(e: any) => {
                                    ...
                                    if (value && value.length > 0) {
                                        clientAllData(`end_date=${debtClientData?.end_date}&start_date=${value}&branch_id=${search?.branch?.value}`)
                                    }
                                }}
                                ...
                                onKeyDown={
                                    (e: any) => {
                                        if (e.key === 'Enter') {
                                            ...
                                            clientAllData(`end_date=${debtClientData?.end_date}&start_date=${debtClientData?.start_date}&branch_id=${search?.branch?.value}&full_name=${e.target.value?.trim().toLowerCase()}`)
                                        }
                                    }
                                }
                                ...
                                onKeyDown={
                                    (e: any) => {
                                        if (e.key === 'Enter') {
                                            ...
                                            clientAllData(`end_date=${debtClientData?.end_date}&start_date=${debtClientData?.start_date}&branch_id=${search?.branch?.value}&phone=${e.target.value?.trim().toLowerCase()}`)
                                        }
                                    }
                                }
                                value={search?.phone}
                                ...
                                onKeyDown={
                                    (e: any) => {
                                        if (e.key === 'Enter') {
                                            ...
                                            clientAllData(`end_date=${debtClientData?.end_date}&start_date=${debtClientData?.start_date}&branch_id=${search?.branch?.value}&person_id=${e.target.value?.trim().toLowerCase()}`)
                                        }
                                    }
                                }
                                ...
                                onKeyDown={
                                    (e: any) => {
                                        if (e.key === 'Enter' && e.target.value?.trim().length > 0) {
                                            ...
                                            clientAllData(`end_date=${debtClientData?.end_date}&start_date=${debtClientData?.start_date}&branch_id=${search?.branch?.value}&data_birth=${e.target.value?.trim().toLowerCase()}`)
                                        }
                                    }
                                }
...

                            {
                                ...
                                render: (value: any, data: any) => {
                                    ...
                                    return <NumericFormat displayType="text"
                                        thousandSeparator
                                        decimalScale={2}
                                        value={total_price - pay_total_price - discount} />
                                }
                            },
                            {
                                title: 'Tolash muddati ',
                                key: 'dedline_date_',
                                render: (value: any, data: any) => {


                                    return <p>
                                        {value?.client_payment?.at(-1)?.payment_deadline ?? '-'}
                                    </p>
                                }
                            },
                            {
                                ...
                                render: (value: any, data: any) => {
                                    ...
                                    return <p>
                                        {value?.client_payment?.at(-1)?.debt_comment ?? '-'}
                                    </p>
                                }
                            },
                            // {
                    ...
                    load ? '' :
                        <Pagination
                            setPageLimit={(e: any) => {
                                ...
                                (clientAllData(`start_date=${debtClientData?.start_date}&end_date=${debtClientData?.end_date}&data_birth=${search?.data_birth}&full_name=${search?.full_name}&person_id=${search?.person_id}&phone=${search?.phone}&page=${1}&per_page=${e}`))
                            }}

                            pageLmit={debtClientData?.per_page}
                            current={debtClientData?.current_page} total={debtClientData?.last_page} count={(e: any) => {

                                (clientAllData(`start_date=${debtClientData?.start_date}&end_date=${debtClientData?.end_date}&data_birth=${search?.data_birth}&full_name=${search?.full_name}&person_id=${search?.person_id}&phone=${search?.phone}&page=${e}&per_page=${debtClientData?.per_page}`))
                            }} />
                }
            </div>
            <CashRegister
