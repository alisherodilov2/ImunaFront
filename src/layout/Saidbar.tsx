...
import { BsChevronLeft, BsChevronDoubleLeft } from 'react-icons/bs'
import { useDispatch, useSelector } from 'react-redux';
import { isFindFunction, isMenuFunction, isSearchFunction } from '../service/reducer/MenuReducer';
import { ReducerType, TypeState } from '../interface/interface';
import { AppDispatch } from '../service/store/store';
import Button from '../componets/buttons/Button';
import { ScaleLoader } from 'react-spinners';
import LoadSingle from '../componets/api/LoadSingle';
import { nanoid } from '@reduxjs/toolkit';
import homeImg from './icon/home.svg'
import logout from './icon/logout.svg'
import CatImg from './icon/category.svg'
import { isLogoutFunction } from '../service/reducer/ProfileReducer';
import {
    Accordion,
    AccordionBody,
    AccordionHeader,
...
const Saidbar = () => {
    ...
    const [open1, setOpen1] = useState('') as any;
    const toggle = (id: any) => {
        if (open1 === id) {
            setOpen1();
        } else {
    ...
    return (
        <>
            <aside id="layout-menu" className={`layout-menu menu-vertical menu bg-menu-theme ${menu ? 'active' : ''}`} data-bg-class="bg-menu-theme">
                <div className="app-brand demo">
                    ...
                    <button
                        ...
                        className="btn text-white layout-menu-toggle menu-link  d-block d-xl-none">
                            ...
                            ?.filter((item: any) => (item?.condition && item?.condition?.length > 0 ? +(user?.department?.[item?.condition] || user?.[item?.condition]) : true) && new Set(item.role).has(user?.role) && item?.link == true)?.map((item: any, index: any) =>
                                item?.children ?
                                    <li className='w-100' key={index}>
                                        <Accordion flush className='w-100' open={open1} toggle={toggle}>
                                            <AccordionItem className='menu-item w-100'>
                                                <AccordionHeader targetId={index} className='menu-link w-100' >
                                                    <i className="menu-icon tf-icons bx bx-copy" />
                                                    <span data-i18n="Extended UI" className='list-inline-item'>{item.label}</span>
                                                </AccordionHeader>
                                                <AccordionBody accordionId={index} className="">
                                                    <ul className="flex menu-inner ">
                                                        {
                                                            item?.children
